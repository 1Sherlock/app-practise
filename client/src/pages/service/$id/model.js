...
export default ({
  ...
  effects: {
    * createOrUpdate({payload}, {call, put, select}){
      const {operator, selectedCategoryId} = yield select(_ => _.service);
      yield put({type: 'updateState', payload: {confirmLoading: true,}});
      const res = yield call(payload.path ? api.updateService : api.createService, {
    ...
    * read({payload}, {call, put, select}){
      const {selectedCategoryId} = yield select(_ => _.service);
      ...
      const res = yield call(api.getService, path ? {path} : payload);
    ...
    * createOrUpdateCategory({payload}, {call, put, select}){
      const {operator} = yield select(_ => _.service);
      yield put({type: 'updateState', payload: {confirmLoadingCategory: true,}});
      const res = yield call(payload.path ? api.updateServiceCategory : api.createServiceCategory, {operator, ...payload});
      if (res.error) {
        notification.error({message: res.error})
      } else {
        notification.success({message: payload.path ? "O'zgartirildi" : "Qo'shildi"})
    ...
    * readCategories({payload}, {call, put, select}){
      ...
      const res = yield call(api.getServiceCategory, operator ? {path: operator} : payload);
    ...
    * deleteCategory({payload}, {call, put, select}){
      const res = yield call(api.deleteServiceCategory, payload);
