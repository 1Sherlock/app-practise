...
exports.get_all_by_category = async (req, res, next) => {
    try {
        const docs = await Sms.findAll({where: {category: req.params.id}, order: [['order', 'ASC']]});
        res.status(200).json({list: docs});
    } catch (err) {
        res.status(500).json({
            error: err
...
exports.update = async (req, res, next) => {
    ...
    if (name && nameRu && nameKr && description && descriptionRu && descriptionKr && ussdCode && order && operator && category) {
        try {
            let ussd = await Sms.findByPk(id);
...
exports.delete = async (req, res, next) => {
    try {
        let ussd = await Sms.findByPk(req.params.id)
        const id = ussd.id;
